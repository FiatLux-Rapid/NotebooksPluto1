...
# ╔═╡ 4258fc8f-11bd-4e2a-88ba-80a41e274efd
using IfElse

# ╔═╡ 474a7c52-38c5-11ed-324a-9b05b845ae7b
using ModelingToolkit

# ╔═╡ 76535166-f16c-4daa-8654-cf2aa4837050
using DifferentialEquations

# ╔═╡ b5edf809-4d6a-4c45-8f0c-6a7610958cdc
using Plots

# ╔═╡ 7c5afaac-5740-42ea-8fd7-aa10567269aa
using OrdinaryDiffEq

# ╔═╡ aec1d63a-f2ac-4977-bac8-80ddb792a033
begin

	
	function f(du,u,pp,t)
	    du[1] = -u[1]+pp
	end
	const tstop1 = [5.]
	const tstop2 = [9.]
	...
	dosetimes = [4.0,8.0]
	pp = 0.0
	conditiond(u,td,integrator) = td ∈ dosetimes
	affectd!(integrator) = integrator.u[1] += 10
	cbd = PresetTimeCallback(dosetimes,affectd!)
	#save_positions = (true,true)

	#cb1 = DiscreteCallback(condition1, affect1!, save_positions=save_positions)
	cb1 = DiscreteCallback(condition1, affect1!)
	save_positions = (false,true)
	
	#cb2 = DiscreteCallback(condition2, affect2!, save_positions=save_positions)
	cb2 =PresetTimeCallback(tstop2,affect2!)

	conditions(u,t,integrator) =    u[1]-17.0  # t-8.0
	affects!(integrator) = terminate!(integrator)
	#cbstop = DiscreteCallback(conditions,affects!)-
	cbstop = ContinuousCallback(conditions,affects!)
    

	
	cbs = CallbackSet(cb1,cb2,cbd,cbstop)
	#cbs = CallbackSet(cb1,cb2)
	#cbd = DiscreteCallback(conditiond,affectd!)
	probd = ODEProblem(f,u0d,(0.0,10.0),pp,tstops=[5.0])
	#sold = solve(probd,Tsit5(),callback=cbd,tstops=dosetimes)
	sold = solve(probd,Tsit5(),callback=cbs)
 	plot(sold,tspan=(0.0,sold.t[end]))


end

# ╔═╡ 58215661-1d00-467a-bd72-b323cadb261f
ODESystem.

# ╔═╡ 1a72d2b7-a8c4-4041-ac9d-84ebbf6863d2
begin
	
 # ton : temps pour la fermeture du switch
	tspansw = (0.0, 1.0e-9)
	 ton=0.3e-9
	 toff=6000e-9
	function affectson!(integrator)
	  integrator.p = 1e-3
	end
	function affectsoff!(integrator)
	  integrator.p = 1e5
	end
	...
	function Switch(;name,ton=ton,toff=toff) 
	    ...
	    @unpack v, i = oneport
		#@variables R
	    ps = @parameters ton=ton 
		#@variables p(t)
		
	
	#p ~ t #ifelse(t>=ton ,1e-3,1e6)
		eqsw = [ v ~ IfElse.ifelse((t>ton) & (t<=toff) ,1e-3*i,IfElse.ifelse(t>toff,1e9*i,1e9*i))]
	...
	@named _rc_modelz = ODESystem(rc_eqsz, t) #; discrete_events = [closed, opened])
	...
	probz = ODAEProblem(sysz, u0z, (0, toff+1e-6))
	solz = solve(probz, Tsit5() ,tstops = [ton, toff])
	plot(solz)
	
end

# ╔═╡ 5b232d03-af0a-4a7f-97b9-74e72e7ff7f4
string(rc_modelz)

# ╔═╡ 39db3c59-b041-4d1d-a58f-63889b7efbbf
string(resistorz)

# ╔═╡ 79e297a5-4816-4db2-b445-fd6d38990512
solve(probz, Tsit5())

# ╔═╡ fa497fd3-1de9-4a6b-80ab-af635fbb7d91
sysz

# ╔═╡ d1ec28d3-83c5-4ff2-9ed5-30f530f30549
begin
	@parameters tkill
...
# ╔═╡ 8c0fea08-0a4e-4f9a-86d7-5c95ca44f7a7
begin
#module myCirc
...
# ╔═╡ 712c7e21-1dd2-4635-bfdd-997c4618fa97
sol(3)

# ╔═╡ 1c34c391-2f1e-4fc4-85af-ae6ef5229d0e
α

# ╔═╡ 19316786-4fce-4f32-8777-b826649b65bb
typeof(eqs)

# ╔═╡ d7a0f4de-ee1e-4b75-ba62-cf468ad1b7cb


# ╔═╡ 10c96bab-edba-47d9-89ed-d6a6f833cb9b


# ╔═╡ a25ca2e1-0162-4874-884c-aa706bf9a02c


# ╔═╡ 42dcfbf5-6d3b-472a-89ec-0568c50d3bba
...
IfElse = "~0.1.1"
ModelingToolkit = "~8.23.0"
OrdinaryDiffEq = "~6.27.1"
Plots = "~1.33.0"
"""
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "245e63056701edd121e62300396e79e88af6f3cc"
...
git-tree-sha1 = "9442dacd31424b624a8717867f4cebaf97f8480d"
