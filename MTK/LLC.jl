...
# ╔═╡ 14e3865b-761c-4bf8-977d-64e434019f27
begin
	import Pkg
 	Pkg.status("ModelingToolkit")
end

# ╔═╡ cf84c977-25f0-4e48-bb2c-72906eeb8f11
begin
	using Plots
...
# ╔═╡ 339760c7-4748-4d90-9cc6-76d31fb3af09
plot(npsol,idxs=[co.v])

# ╔═╡ d5e2973b-adad-48b2-a2a2-2d5b67d242c2
begin
	
	
	clength=3 #in meter  5ns/m typical
	nloop=20
	prfnp=1e5  # pulse repetition frequency
	nbmacropulse=1
	tpulsenp=15e-9  # pulse duration
	typenp="bipolar"  # monopolar pulse other option bipolar pulse "bipolar" 
	nbpulsenp=3 # total number of number of pulses
	#linelengthnp=5 # 
	swonnp=5e-9 #10e-9 #5e-9  # initial delay
	swtrnp=10e-9  #rise and fall time
	...
	inouttime=5e-9*clength*2
	swronnp=1e-3
	swroffnp=1e6
	
	#crowbar commutes after initial pulse closing until n reflexion occurs 
	
	crbswtoffnp1=swtoff+inouttime*(nbpulsenp-1)+inouttime/2

	tstop0=[]
	for i in 0:nbmacropulse-1
	push!(tstop0,swonnp+i*1/prfnp)
	push!(tstop0,swtoff+swtrnp+i*1/prfnp)
	push!(tstop0,crbswtoffnp1+i*1/prfnp)
	end
	

	@named nppulsesw=Switch2ways(ton=swonnp,twidth=tpulsenp,rt=swtrnp,ron=1e-3,roff=1e6,prf=prfnp)
	@named cbsw=Switchrec(ton=crbswtoffnp1,toff=1.0,ron= 50,roff=1e6,prf=prfnp)
	@named cbsw1=Switchrec(ton=swtoff,toff=crbswtoffnp1,ron=crbr,roff=1e6,prf=prfnp)
	@named linenp=line(name="linenp",nloop=nloop,clength=clength)
	@named vinnp=Capacitor(C=1)  #ConstantVoltage(V=5e3)    
	@named rcircuitnp=Resistor(R=0.5e3)	
	@named gnp=Ground()
	@named co=Capacitor(C=40e-12) 
	connection=eval(Meta.parse("
 	nprc_eqs = [
	          connect(vinnp.p, nppulsesw.p)
			  connection
			  connect(nppulsesw.n, linenp.li_1.p,cbsw.p,cbsw1.p)
	     
	         # connect( gnp.g,vinnp.n,cbsw.n,cbsw1.n,co.n,rcircuitnp.n)
			connect( gnp.g,vinnp.n,cbsw.n,cbsw1.n,co.n,linenp.ci_1.n,rcircuitnp.n)
	         ]
	
	@named np_rc_model = ODESystem(nprc_eqs, t) 
	@named nprc_model = compose(np_rc_model, [gnp,vinnp,cbsw,cbsw1,nppulsesw,linenp,co,rcircuitnp])
		...
		npsol = solve(npprob, Tsit5(),tstops=tstop0)

	nothing

end

# ╔═╡ 36ed7ffa-75fd-4002-a77a-4d49c960814c
begin

	
	
	function line(clength=3,nloop=20)
		
		ps = [] #@parameters clength=3 nloop=20
		sts = [] #@variables vin(t)=0 iin(t)=0 vout(t)=0 iout(t)=0

		#@named l_in = OnePort()
		#@named l_out = OnePort()
		@named li 1:nloop-1 i -> Inductor(L=2.5229658792650916e-7*clength/nloop)
		@named ci 1:nloop-1 i -> Capacitor(C=1.0104986876640419e-10*clength/nloop)
		@named li_end=Inductor(L=2.5229658792650916e-7*clength/nloop)
		rc_eqs2=[]
	    vari=[]
		for i in 1:nloop-2
			...
			push!(rc_eqs2,connect(li[nloop-1].n,ci[nloop-1].p,li_end.p))
        	push!(vari,li_end)
		for i in 2:nloop-1
        ...
        @named linenp =ODESystem(rc_eqs2,t)
		
	#eval(Meta.parse(name*"=compose(ODESystem(rc_eqs2,t,name=:"*name*"),"*vari*")")) 
		
		
		compose(linenp,vari)#[li[1],ci[1],li[2],ci[2],li[3],ci[3],li[4],ci[4],li[5],ci[5],li[6],ci[6],li[7],ci[7],li[8],ci[8],li[9],ci[9],li[10],ci[10],li[11],ci[11],li[12],ci[12],li[13],ci[13],li[14],ci[14],li[15],ci[15],li[16],ci[16],li[17],ci[17],li[18],ci[18],li[19],ci[19],li[20]])
  
	end
	
end

# ╔═╡ bf086336-b8ae-4b7f-97a1-ff0358bc0f27
linenp.ci[1]

# ╔═╡ a27a536a-aaef-4006-bde3-ad356c128e57
begin
...
# ╔═╡ 3b0d9ac2-b6c1-42fd-bb7b-2bad921c7e8b
...
# ╔═╡ 28d9c0d7-4f6d-431b-8479-cdb72b4ee9ec
function llcfonction2(x;crv = 0.0, cov = -volt_out/2,tr0lri =100.0,tr0lm0i =100.0, tr0transf0tpv=volt_out,d20dv = -volt_out,d30dv = volt_out,tr0grounds0gi=0.0)
		...
		vcoave=[]
		   try
			@named	sour0 = Vdfvoltage(d=d,f=f)
	
  			rc_eqsllc0 = [
			connect(sour0.p,tr0.lr.p)
			connect(tr0.lm0.n,cr.p)
			connect(cr.n,sour0.n)
	 
			connect(tr0.transf0.ts.p,d10.d.p,d20.d.n)
			connect(tr0.transf0.ts.n, d30.d.p,d40.d.n)
			connect(d10.d.n,d30.d.n,rs.p)
			connect( d20.d.p,d40.d.p,co.n,rl.n)
			connect(rs.n,co.p,rl.p)
	        connect(tr0.groundp0.g,tr0.grounds0.g)]
		
			@named _rc_model_llc0 = ODESystem(rc_eqsllc0, t)
			@named rc_model_llc0 = compose(_rc_model_llc0,[sour0,tr0,cr,co,rs,rl,d10,d20,d30,d40])
			...
			u0llcv1 = [
		    	...
		    	cr.v => crv
				co.v => cov 
			    tr0.transf0.tp.v=>tr0transf0tpv
				d20.d.v => d20dv
				d30.d.v => d30dv
				tr0.grounds0.g.i=>tr0grounds0gi
     ]

	u0llc1=[]

    probllc0 = ODAEProblem(sys_llc0, u0llcv1, (0, tscano)) #,jac=jacob)
	    solllc0 = solve(probllc0,TRBDF2(),maxiters=1e8,abstol=1e-9,reltol=1e-9)	
    	solt(t)= solllc0(t)[4]
		vcoave=mean(solt.(tscano-0.5/f:1e-6:tscano))  
		tr0lm0iini=solllc0(0.0)[1]
		tr0lriini=solllc0(0.0)[2]  
		crvini=solllc0(0.0)[3]
		covini=	solllc0(0.0)[4]  
		tr0transf0tpvini=solllc0(0.0)[5]
		d20dvini=solllc0(0.0)[6]
		d30dvini=solllc0(0.0)[7]	
		tr0grounds0giini=solllc0(0.0)[8]
			   
		catch
		
		end  #try 
		
	return vcoave,f,d,[tr0lm0iini,tr0lriini, crvini,covini ,tr0transf0tpvini,d20dvini, d30dvini,tr0grounds0giini]
end

# ╔═╡ dcc50342-0199-4bf2-97ab-d39011703a9b
begin	
	#=
	Modules: 
	Pin(; name)
	...
	SinVoltage(;name, V = 1.0,freq=1.0)
	Inductor(;name, L = 1.0)
	Transformer(;name,ratio=1.0)
	CoupledInductor(;name,coupling=0.1,ratio=1.0,Lp=1.0)
	Diode(;name) 
	line(;name="myname")   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	VoltageSensor(; name)  ! inutile
	Switch(;name,ton=ton,toff=toff,ron=ron,roff=roff) 
	Switchrt(;name,ton=ton,twidth=twidth,rt=rt,ron=1e-3,roff=1e6) 
	SwitchET(;name,ton=ton,twidth=twidth,rt=rt,ron=1e-3,roff=1e6,rdiode=1e-3)
	Switchrec(;name,ton=ton,toff=toff,ron=ron,roff=roff,prf=1e7) 
	switch2ways(;name,ton=ton,twidth=twidth,rt=rt,ron=1e-3,roff=1e6,prf=1e5)
...
function ConstantVoltage(;name, V = 1.0)
	...
	function SinVoltage(;name, V = 1.0,freq=1.0)
		@named oneport = OnePort()
	    @unpack v = oneport
...
function Transformer(;name,ratio=1.0)
	...
	function Diode(;name) # 0 bloqué, 1 passant i>0 
	...
	function line(;name="myname")
		local nloop=5
		local clength=1.0
		l_in=OnePort(name=:l_in)
		l_out=OnePort(name=:l_out)
		local vari=""
		for i in 1:nloop-1
			vari*="li["*string(i)*"],ci["*string(i)*"],"
		end
		vari*="li["*string(nloop)*"],"
		vari*="ci["*string(nloop)*"]"

		
		for i in 1:nloop
			try
				eval(Meta.parse("@named li[i]=Inductor(L=2.5229658792650916e-7*"*string(clength/nloop)*")"))
			catch
			end
			try
				eval(Meta.parse("@named ci[i] =Capacitor(C=1.0104986876640419e-10*"*string(clength/nloop)*")"))
			catch
			end
		end
	...
	#	push!(rc_eqs2,connect(li[nloop].p,ci[nloop].p))
		try
		push!(rc_eqs2,connect(li[nloop].n,ci[nloop].p))
		catch
		end
		try
			push!(rc_eqs2,connect(ci[nloop-1].n,ci[nloop].n))
		catch
		end
	 # eval(Meta.parse(name*"=compose(ODESystem(rc_eqs2,t,name=:"*name*"),"*vari*")"))
	rc_eqs2
	...
	#@named level = VoltageSensor()
		...
		function Switch(;name,ton=ton,toff=toff,ron=ron,roff=roff) 
		@named oneport = OnePort()
	    @unpack v, i = oneport
	   sts= []#@variables t rsw
	...
	function Switch2ways(;name,ton=ton,twidth=twidth,rt=rt,ron=1e-3,roff=1e6,prf=1e5)
...
# ╔═╡ e13e6380-4478-11ed-1235-f951f61288f9
begin
	#=import Pkg
	Pkg.update()
	Pkg.add("Plots")
...
git-tree-sha1 = "e576fcd634f3f9c6c5920cb8fe63707a80b78012"
