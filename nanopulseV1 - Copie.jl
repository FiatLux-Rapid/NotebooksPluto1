...
begin
	prfnp=1e6  # pulse repetition frequency
	nbmacropulse=2
	...
	swonnp=0 #10e-9 #5e-9  # initial delay
   ...
   # @named nppulsesw=Switchrt(ton=swonnp,twidth=tpulsenp,rt=swtrnp,ron=1e-3,roff=1e6)
	@named nppulsesw=BiswitchET(ton=swonnp,twidth=tpulsenp,rt=swtrnp,ron=1e-3,roff=1e6,prf=prfnp)
	...
	@named nprc_model = compose(np_rc_model, [gnp,rcircuitnp,vinnp,cbsw,cbsw1,nppulsesw,linenp,vprobenp])
	   ...
	   # plot(npsol,idxs=[linenp.cout.v],tspan=(0.0,(nbmacropulse-1)/prfnp+crbswtoffnp1+2*inouttime)) 
end
...
# Check variable
	npsol
	cheksolstr="npsol"
	checkvarstr="rcircuitnp" #vinnp" #rcircuitnp" # #nppulsesw" #cbsw"  #linenp.cout"  #  nppulsesw
	checkvar=eval(Meta.parse(checkvarstr))  #
	checksol=eval(Meta.parse(cheksolstr))  #
	plot(checksol,idxs=[checkvar.i,checkvar.v],layout = grid(2, 1, heights=[0.5 ,0.5])) #ylimits=(0,12000))
end
...
# ╔═╡ e9b79439-5afd-42ab-817a-bef800101a21
begin
	plot(0:1e-10:checksol[t][end],checkinti.(0:1e-10:checksol[t][end]))
	plot!(checksol,idxs=[checkvar.i])
end

# ╔═╡ 599c5c67-9baf-484a-b4a6-0d99ece40643
quadgk(checkpower, 0, min(checksol[t][end],1/prfnp), rtol=1e-5)[1]*prfnp

# ╔═╡ 8e5fd3e6-2dd2-42e8-9970-c89821ba98eb
min(checksol[t][end],1/prfnp)
...
# Deux autres switchs swcc,sw50 pour traiter les réflexions (CC et 50 ohm)
	...
	@named rc_modelnp = compose(_rc_modelnp, [groundnp,rcircuit,vin,sw50,swpulse,npline,vprobe])
		...
		probnp = ODAEProblem(sysnp, u0np, (0, 2*toff50)) #,pini)
...
plot(solnp,idxs=[npline.cout.v],ylimits=(0,1.1e4))
